...
class Viewer:
    ...
    def create_explorer(self):
        with dpg.window(autosize=True, min_size=[220, 100], no_close=True, max_size=viewport_size, user_data=0) as self.explorer:
            try:
                # visualize selection
                ...
                for item in self.items:
                    ...
                    if not dpg.does_item_exist(patient_id):
                        patients.append(dpg.add_tree_node(tag=patient_id, label=patient_id, user_data=0))

                    if not dpg.does_item_exist(study_id):
                        studies.append(dpg.add_tree_node(parent=patient_id, tag=study_id, user_data=0,
                ...
                dpg.configure_item(self.explorer,
                                   label=f'Explorer {label_summary(0, len(patients), dpg.get_item_user_data(self.explorer))}')
    ...
    def run(self):
        ...
        with dpg.viewport_menu_bar():
            dpg.add_menu_item(label="Close", callback=dpg.destroy_context)
...
def callback_item(sender, _, user_data: dict):
    ...
    with dpg.window(label='/'.join(labels), autosize=True, on_close=lambda: callback_items.pop(sender)) as w:
        ...
        # preview
        sample = os.path.join(path, user_data['Sample'])
        ...
        try:
            ...
            with dpg.collapsing_header(label=f'Sample: {user_data["Sample"]}', default_open=True):
